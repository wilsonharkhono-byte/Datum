// Pure schedule math — no "use server", no Supabase. Lives apart from
// schedule.ts because that file is a "use server" module (server actions),
// where every export must be an async function; these synchronous helpers
// would poison the entire server-action graph if exported from there.
