...
export function MessageList({ messages, pending }: { messages: Message[]; pending: boolean }) {
  ...
  return (
    <div ref={ref} className="flex-1 space-y-2 overflow-y-auto px-4 py-2 text-sm">
      ...
      {messages.map((m, i) => {
        if (m.role === "user") {
          return (
            <div key={i} className="flex justify-end">
              <div className="max-w-[70%] rounded bg-green-100 px-3 py-1.5">{m.content}</div>
            </div>
          );
        }
        if ("proposal" in m) {
        ...
        return (
          <div key={i} className="flex flex-col gap-2">
            <div className="max-w-[80%] rounded border border-stone-200 bg-white px-3 py-1.5">{m.content}</div>
            {m.citations?.length ? (
      ...
      {pending ? <div className="italic text-stone-500">…sedang memproses</div> : null}
    </div>
  );
}
