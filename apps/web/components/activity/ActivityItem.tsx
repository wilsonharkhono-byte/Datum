  ...
  return (
    <li className="rounded border border-[var(--border)] bg-white p-3 text-xs">
      <div className="mb-1 flex flex-col gap-0.5 text-[10px] sm:flex-row sm:items-center sm:justify-between sm:gap-2">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <Link href={`/project/${item.projectCode}`}
            className="font-semibold uppercase tracking-wide text-[var(--sand-dark)] hover:underline">
            {item.projectCode}
          </Link>
        </div>
      ...
      <div className="text-sm text-[var(--foreground)]">
        ...
        <span className="ml-1 text-[var(--text-secondary)]">— {item.detail || "(tidak ada detail)"}</span>
      </div>
    </li>
  );
}
