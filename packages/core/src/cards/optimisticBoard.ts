...
/** Build the ghost card shown immediately on submit, before the server insert
    completes. Pass a unique `id` (generated by the caller using the platform's
    UUID API) to avoid React key collisions when the same title is added multiple
    times. When `id` is omitted the deterministic fallback
    `optimistic:${topicId}:${title}` is used (SSR-safe, stable across renders).
    The card is replaced by the real row when the server action settles and fresh
    `board` data arrives. */
